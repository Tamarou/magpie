...
implement data access via a RESTful API and so you reach for one of the popular tools that purports to make the process simple. The early stages of the project are promising. 
...
Soon, you find that the hands-off "connect the database to the Web" application you thought you were building is turning into a series hacks, work-arounds, and one-offs. The tool that was supposed to make things easier now actually ties your hands in certain very common ways. You're wondering why this all seems so hard and you begin to suspect that REST is just another in a long line of overcomplicated bondage fads that look great on paper but fall to pieces when you try to do anything substantial. 
